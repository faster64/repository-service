...
{
    public class UpdateClockingValidator : BaseClockingValidator<Clocking>
    {
        #region Properties
        ...
        public UpdateClockingValidator(
        ...
        protected void ValidateShift(long targetShiftId, long oldShiftId)
        {
            RuleFor(clocking => clocking)
                ...
                {
                    var oldShift = await _shiftReadOnlyRepository.FindByIdAsync(oldShiftId);
                    ...
                    {
                        var targetShift = await _shiftReadOnlyRepository.FindByIdAsync(targetShiftId, false, true);
        ...
        protected void ValidateClockingOverlapTimeAsync(DateTime startTime, DateTime endTime)
        {
            RuleFor(clocking => clocking)
               .MustAsync(async (clocking, token) =>
               {
                   ...
                   var clockings = await _clockingReadOnlyRepository.GetBySpecificationAsync(existClockingOfEmployeeInShiftTargetSpec);
